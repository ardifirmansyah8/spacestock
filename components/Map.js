...
const Map = ({ data, markerClick }) => {
  const [viewport, setViewport] = useState({
    longitude: data instanceof Array ? 106.816666 : data.address.longitude,
    latitude: data instanceof Array ? -6.37 : data.address.latitude,
    zoom: data instanceof Array ? 10 : 13,
    width: '100%',
    height: '100%',
    bearing: 0,
    pitch: 0,
        ...
        data.map(place => (
          ...
          >
            <svg
              ...
              onClick={() => markerClick(place.name)}
            >
              <path
